        ...
        <div class="col-md-12">
            @can('attendances_report_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success d-none" href="{{ route('frontend.attendances-reports.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $attendancesReport->id }}">
                                        ...
                                        <td>
                                            @can('attendances_report_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.attendances-reports.show', $attendancesReport->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('attendances_report_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.attendances-reports.edit', $attendancesReport->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('attendances_report_delete')
                                                <form action="{{ route('frontend.attendances-reports.destroy', $attendancesReport->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('attendances_report_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.attendances-reports.massDestroy') }}",
